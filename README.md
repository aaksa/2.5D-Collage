...
- **Browser bar.** A Chrome-style frame across the top of frame, dark
  theme (`components/BrowserChrome.tsx`): a tab strip with one open tab
  and a toolbar whose address field shows `localhost:62`, for the whole
  film. An overlay scrollbar on the right moves down
  as the film plays, as if the page were being scrolled.
